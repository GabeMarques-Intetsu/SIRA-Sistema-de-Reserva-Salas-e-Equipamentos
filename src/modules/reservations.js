...
function buildRow(r, tbody) {
  ...
  if (r.status === 'pending') {
    actionsCell.appendChild(
      btn('Editar', 'btn-sm', () => openEditModal(r, tbody)),
    );
    actionsCell.appendChild(
      btn('Cancelar', 'btn-sm btn-danger', () =>
        deleteReservation(r.id, tbody),
      ),
    );
    ...
    actionsCell.appendChild(
      btn('Recorrer', 'btn-sm', () =>
        toast('Recurso enviado ao coordenador.', 'success'),
      ),
    );
  }
...
function openEditModal(r, tbody) {
  const purposeInput = el(
    'textarea',
    { class: 'form-input', rows: '3', style: 'resize:none' },
    r.purpose,
  );
  const timeSelect = el(
    'select',
